...
resource "aws_iam_role" "iam_for_lambda" {
  name               = join("-", [var.platform_name, var.account_name, var.function_name, "role", var.env])
  assume_role_policy = data.aws_iam_policy_document.assume_role.json
}


resource "aws_iam_policy" "policy_for_lambda" {
  name   = join("-", [var.platform_name, var.account_name, var.function_name, "policy", var.env])
...
resource "aws_lambda_function" "parameter_replication" {
  ...
  function_name = join("-", [var.platform_name, var.account_name, var.region, var.function_name, var.env])
